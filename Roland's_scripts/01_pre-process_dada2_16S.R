sink(file="./console_output/01_console_output.txt")  ## open sink connection to write console output to file
...
fnFs <- sort(list.files(data.fp, pattern="_R1", full.names = TRUE))
fnRs <- sort(list.files(data.fp, pattern="_R2", full.names = TRUE))
...
for (i in seq_along(fnFs)) {
  system2(cutadapt, args = c("-j", 0, R1.flags, R2.flags, "-n", 2, # -n 2 required to remove FWD and REV from reads
			     "--discard-untrimmed", # remove reads without primers
			     "-e 0.05", "--overlap 10", # max error in primer sequence in 0.05%, minumim overlap is 10 bases
