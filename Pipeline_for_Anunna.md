...
``` bash
# Handy tricks
  #^C			              #Kills job in console
  #Highlight code       #copy code
  #Right mouse click    #paste 	
  #Arrow up             #reuse previous lines of code
  #Tab complete         #finish code or file path

# Basic commands
pwd 		                #print working directory
cd			                #change working directory to home
cd /directory_name	    #change working directory up
cd ..		                #Change working directory down
cd ../directory_name/   #change working directory to parallel directory
ls			                #print list of file
ls -l		                #print long list of file
cat		                  #print file
cat > test		          #make new file
zcat		                #print compressed file
head –n 40		          #print head (first 40 lines) of file
nano		                #open file editor
nano test		            #make new file and directly edit
mkdir 		              #create directory
mv 		                  #move and/or rename file or directory
cp			                #copy file
cp -r		                #copy directory
rm			                #remove file
rm --help		            #open help file rm
rm -r		                #remove directory
rmdir		                #remove directory
rm -r test test.txt	    #remove multiple directories
ls > filenames		      #save list of files names in file
wc -l filenames	        #count number of lines in file
ls | wc -l		          #count number of lines (files) in file list

# Regular expressions
cp Directory1/F* Directory2/
grep -e "C.*[0-9]" -e "Blank" FileNames_withprimes

#Running slurm scripts
sbatch		            #Run slurm script
squeue -u kreek001	  #Check progress of running slurm script
```
...
## Run R code in HPC with slurm 
You can run R code on the HPC. First you create an R file (or load the R file into the HPC).   
   

``` bash
nano testscript.R #Create new R script and directly open text editor interface
```
   
Now you can add lines of code. A simple example:   
...
You can run this R script inside a slurm script. Slurm scripts have commands starting with `#SBatch` that indicate the required time, memory, nodes, etc. You can also specify the job name and name of the output file that includes the results of the script. In the slurm script, you have to specify which R software version you want to use. See more information on that topic [later in this file](#accessing-software-and-installing-packages). You first have to construct a slurm script (or load the slurm file into the HPC).   
   

``` bash
nano testscript.slurm #Create new slurm script and directly open text editor interface
```
...
#SBATCH --job-name='testscript'       ### you can give your job a name
#SBATCH --output=testscript_%j.output ### name of the output file

module load 2024
module load R/4.4.2-gfbf-2024a

Rscript testscript.R # run R script
```
   
You can run (submit a job) the script with `sbatch` command and check the progress of the job with `squeue`.   
   

``` bash
sbatch testscript.slurm

squeue -u kreek001 # change to your own user name
```
   
An output file will be created in your current working directory with the results from the slurm and r script. Think about where you want to store your output and make that your current working directory (navigate there with `cd`) before submitting a job. You can print the output with `cat`.


``` bash
cat testscript_12345678
```
...
**R script**   
For installing packages, you need to change the r scripts. See more information at `Accessing software and installing packages` below.   
...
Rscript ../R/00_install_dada2_tutorial_16S.R
...
I define the directories where the r packages are stored in every r script that I use in the following pipeline.
...
``` bash
#!/bin/bash         ### -login is essential to activate conda environment
...
In here you will filter out those reads that don't match your criteria based on the quality plots from script 02. `truncLen` cuts off sequences at the end (you may want this as the quality of sequences drops at the end) and filters away sequences that are shorter than the values you give for forward and reverse sequences c(for, rev). `maxEE` filters reads on quality. The `filterAndTrim` function can do more like cutting base pairs from the beginning or the end of the sequence. See the help file of this function for more information.
...
See below extra things that need to be changed in this R script!
...
Trunking a bit more with `cutAndTrim()` function in script 03 may increase the quality of the ends of the sequences. This can improve merging and reduce the loss of sequences at the merging step. Keep in mind that the forward and reverse sequence should still overlap after trunking otherwise sequences cannot match. The forward and reverse sequence should together be longer than 460 bp.
...
When you have a trained classifier, set of 16S and ITS primers used. An example is given in the table below. Novogene used slightly different primers for the same 16S region. 
...
To run Qiime2 you need to [install it, and activate within a conda environment inside the HPC](https://use.qiime2.org/en/2025.4/tutorials/intro.html) (Check whether this is still the newest version) if you don't have it already installed. The instructions are straightforward, but most likely this step will take you time as you'll have to wait some time for conda and plug-ins to get downloaded (so take it into account).
