...
You can run this R script inside a slurm script. Slurm scripts have commands starting with `#SBatch` that indicate the required time, memory, nodes, etc. You can also specify the job name and name of the output file that includes the results of the script. In the slurm script, you also have the specify which R software versions you want to use. See more information on that topic [later in this file](#accessing-software-and-installing-packages). You first have to construct a slurm script (or load the R file into the HPC).   
