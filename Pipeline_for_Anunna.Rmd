...
This document was mainly written by Marcela Aragon and Kris de Kreek but gathers tips and tricks given by Pedro Beschoren da Costa and Roland Berdager from WUR, as well as several online resources. 
...
Now comes the part in which you have to remove chimeras and assign taxonomy to your ASVs according to a database (Step 3 in the [Ernakovich pipeline](https://github.com/ErnakovichLab/dada2_ernakovichlab)). For this we use the [SILVA rRNA database](https://www.arb-silva.de/) which is a huge database that matches taxonomy with the sequence of the small subunit of ribosomal RNA (16S/18S). You'll have to download the most updated database file from the SILVA webpage, in the "Downloads" tab under the "arb files" section (https://www.arb-silva.de/download/arb-files/). The file is a bit hidden, as you'll have to click on the "File Archive" link as shown highlighter in the picture below: 


![Fig.X Finding SILVA database](images/06_SilvaDatabase.JPG)

Once there you'll have a long list of releases, go to the latest one, in our case "release_138.1" and then click on the "[Exports]" folder. In there you'll have another long list with many options, you can have a look at the "README.txt" file to see what the different names stand for.

![Fig.X Codes for different types of files](images/06_SilvaDatabase_README.JPG)

We chose the `SILVA_138.1_SSURef_tax_silva_trunc.fasta.gz` option as we want the reference database to be truncated which means that contains only the nucleotides for the 16S gene. **Not sure about difference between full_trunc and just _trunc though**. 

![Fig.X Finding FASTA file of SILVA database](images/06_SilvaDatabase_README_2.JPG)


After the download (about ~3 minutes) you'll need to transfer the file to the terminal using Filezilla as we explained before in **Section X**. The file is large, so it takes a couple of mins for it to be transfered. Be wary of where you store the  file in your HPC directory as you'll need to specify the path later on in R script file. 


![Fig.X Transfering SILVA database](images/06_SilvaDatabase_transfer.JPG)


As now it's a `.qz` file you'll need to decompress it to a `.fasta` file using the command `gunzip -c SILVA_138.1_SSURef_tax_silva_trunc.fasta.gz`, you can check [this webpage](https://www.cyberciti.biz/faq/unpack-tgz-linux-command-line/) for instructions of how to unpack files from the command line. After you press Enter your terminal will go a bit crazy with a rain of A, C, T anfd G's as all the sequences available in the database are being decompressed, you'll have to wait for some good **30 minutes**.  
