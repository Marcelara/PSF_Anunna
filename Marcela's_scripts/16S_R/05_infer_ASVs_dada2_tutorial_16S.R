...
for(sam in sample.names) {
  ...
  dadaF <- dada(derepF, err = errF_4, multithread = TRUE) #this is were the error option is added for F 
  ddF[[sam]] <- dadaF
  # Dereplicate reverse reads
  derepR <- derepFastq(filtRs[[sam]])
  # Infer sequences for reverse reads
  dadaR <- dada(derepR, err = errR_4, multithread = TRUE) #and here for R
  ddR[[sam]] <- dadaR
