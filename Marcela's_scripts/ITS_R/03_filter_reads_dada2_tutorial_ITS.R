...
#PEDRO's addition:to remove truncation (our reads are very shrot) and reduce max ee to 1 (we have good quality)  
filt_out <- filterAndTrim(fwd=file.path(subF.fp, fastqFs), filt=file.path(filtpathF, fastqFs),
                          ...
                          minLen=50, maxEE=c(1,1), truncQ=2, maxN=0, rm.phix=TRUE, #[MA] removed TruncLen and add minLen=50 to remove very low-length sequences
                          compress=TRUE, verbose=TRUE, multithread=FALSE)  #changed to FALSE as there was a problem before 
